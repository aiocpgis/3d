...
async function pollinationsPost(prompt,signal){const response=await fetch('https://text.pollinations.ai/openai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'openai',messages:[{role:'system',content:'You generate concise professional prompts for software developers. Return only the final prompt.'},{role:'user',content:prompt}],temperature:.72,max_tokens:900}),signal});if(!response.ok)throw new Error(`Pollinations POST failed: ${response.status}`);const contentType=response.headers.get('content-type')||'';if(contentType.includes('application/json')){const data=await response.json();return data?.choices?.[0]?.message?.content||data?.choices?.[0]?.text||data?.content||JSON.stringify(data)}return response.text()}
async function pollinationsGet(prompt,signal){const url=`https://text.pollinations.ai/${encodeURIComponent(prompt)}?model=openai`;const response=await fetch(url,{signal});if(!response.ok)throw new Error(`Pollinations GET failed: ${response.status}`);return response.text()}
async function generateWithPollinations(prompt){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),28000);try{try{return await pollinationsPost(prompt,controller.signal)}catch{return await pollinationsGet(prompt,controller.signal)}}finally{clearTimeout(timer)}}
function setPreview(card,text,source){const preview=card.querySelector('.prompt-preview');if(!preview)return;preview.textContent=text;preview.classList.remove('ai-placeholder');preview.classList.add('ai-compact');card.dataset.aiPrompt=text;card.dataset.aiSource=source||'ai';const copy=card.querySelector('.copy-prompt-btn');if(copy){copy.textContent='Copy AI Prompt';copy.classList.add('ai-ready')}}
function setPlaceholder(card){if(card.dataset.aiPrompt)return;const preview=card.querySelector('.prompt-preview');if(!preview)return;preview.textContent='اضغط Generate with AI لتوليد برومبت مخصص عبر Pollinations AI. إذا فشل الاتصال سيبقى البرومبت المحلي متاحًا كخطة احتياطية.';preview.classList.add('ai-compact','ai-placeholder')}
